...
const getApiRecipes = async () => {
    ...
    const result = await axios.get(
        `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&addRecipeInformation=true&number=99`, config
        );
    ...
    for (let i = 0; i < finalResult.length; i++) {
            ...
            sRecipe.dietType = finalResult[i].diets;
            ...
            for ( let j = 0; j < steps.length; j++) {
                ...
                allSteps[stp[k].number] = stp[k].step;
                };
            };
